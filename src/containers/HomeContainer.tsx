...
import ProjectRow from '../components/Common/ProjectRow/ProjectRow';
...
const Home = ({ site }: THomeViewProps) => {
  // useDispatch to get home page
  const matches = useMediaQuery('(max-width:959px)');
  const classes = useStyles();
  const projects = (site?.flocks.find((flock) => flock.slug === 'portfolio')?.data as IProject[]) || [];
  ...
  return (
    <>
      ...
      {/* Intro */}
      <Grid container direction={matches ? 'column-reverse' : 'row'} className={classes.grid} spacing={2}>
